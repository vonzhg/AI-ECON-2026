...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Learning Endogenous Boundaries}\label{sec:boundary_learning}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The debt sustainability score $A_B$ requires knowing the feasible debt range $\Omega_B(\mu', g')$, which is itself defined by the set of admissible states. This \textbf{circular dependency} is resolved via an iterative fixed-point procedure.

\subsubsection{The Circularity Problem}
\begin{itemize}
    \item To compute $\mathcal{A}(s)$, we need $\Omega_B(\mu', g')$.
    \item To estimate $\Omega_B$, we need to identify admissible states (where $\mathcal{A}(s) > \tau_{\text{adm}}$).
\end{itemize}

\subsubsection{Resolution via Iterative Refinement}
We break the circularity through the following procedure, executed periodically during training.
...
\paragraph{Step 1: Generate Reference Grid.}
...
\paragraph{Step 2: Fixed-Point Iteration.}
For $n = 1, \ldots, N_{\text{refine}}$ (typically $N_{\text{refine}} = 3$):
\begin{enumerate}
    \item \textbf{Score:} Compute $\mathcal{A}(s)$ for all $s \in \mathcal{G}_{\text{ref}}$ using current boundaries $\Omega_B^{(n-1)}$.
...
\textit{Convergence Logic:} As boundaries tighten around the true safe set, marginal points lose admissibility, further refining the boundary until stabilization.

\paragraph{Step 3: Final Synchronization.}
...
\paragraph{Stage 2: Feasibility via Supervised Learning.}
...
\end{itemize}
This forces the policy to output ``boundary'' values at infeasible states, preventing value extrapolation.
...
\end{equation}
where $V_{\text{penalty}}$ is a large negative constant (e.g., $-50$) preventing high-value extrapolation into infeasible regions.

Target datasets are regenerated every $N_{\text{draw}}$ epochs.
...
\State Initialize: Scorer $\mathcal{A}$, History $\mathcal{H} \gets \emptyset$, Bounds $\Omega_B^{(0)} \gets [B_{\min}^{\text{init}}, B_{\max}^{\text{init}}]$

\For{$k = 1$ \textbf{to} $K$}
    \State \textbf{// --- Phase Transition ---}
    \If{$k = K_w$}
        \State Build initial score cache via Iterative Refinement (Section~\ref{sec:boundary_learning})
    \EndIf
    ...
    \Else \Comment{Adaptive}
        ...
        \State Compute scores $\mathcal{A}(s)$ for all $s \in \mathcal{S}_{\text{cand}}$
    ...
    \State
    \State \textbf{// --- Step 5: Boundary Refinement ---}
    \If{$k \geq K_w$ \textbf{and} $(k - K_w) \mod f_{\text{cache}} = 0$}
        \State Run Iterative Refinement (Section~\ref{sec:boundary_learning})
        \State Update $\Omega_B(\mu, g)$ boundaries
    \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
...
A learned domain classifier requires ground-truth labels, which are unavailable since feasibility is endogenous. The admissibility scorer provides economically-grounded signals without supervision.

\paragraph{Why Two-Stage Policy Training?}
Stage 1 optimizes for welfare in the safe interior. Stage 2 explicitly teaches boundary behavior, preventing the value function from erroneously extrapolating high values into infeasible regions.
...
\begin{itemize}
    \item $\tau_{\text{high}}, \tau_{\text{low}}$: Partition thresholds affecting safe/boundary/fail classification
    \item $\alpha$: Quantile parameter for boundary estimation (robustness vs. coverage)
    \item $N_{\text{refine}}$: Iterations for boundary fixed-point (typically 3 suffices)
