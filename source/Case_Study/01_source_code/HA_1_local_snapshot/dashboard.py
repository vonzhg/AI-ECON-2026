...
def sample_from_boundary(num_samples, boundary, config, device):
    ...
    if boundary.admissible_points is not None and len(boundary.admissible_points) > 10:
        ...
        # Add perturbations
        scale = 0.05
        K_samples = np.clip(base_points[:, 0:1] + np.random.randn(n_base, 1) * (sb['K_max'] - sb['K_min']) * scale,
                           sb['K_min'], sb['K_max'])
        ae_samples = np.clip(base_points[:, 1:2] + np.random.randn(n_base, 1) * (sb['a_max'] - sb['a_min']) * scale,
                            sb['a_min'], sb['a_max'])
        au_samples = np.clip(base_points[:, 2:3] + np.random.randn(n_base, 1) * (sb['a_max'] - sb['a_min']) * scale,
                            sb['a_min'], sb['a_max'])
        ce_samples = np.random.rand(n_base, 1) * (sb['c_max'] - sb['c_min']) + sb['c_min']
        cu_samples = np.random.rand(n_base, 1) * (sb['c_max'] - sb['c_min']) + sb['c_min']
        
        candidates = np.hstack([K_samples, ae_samples, au_samples, ce_samples, cu_samples])
...
def train():
    ...
    viz = HAVisualizer(config, device)
    ...
    for iteration in range(num_iters):
        ...
        with torch.no_grad():
            ...
            if out is not None:
                scores = model.compute_admissibility(out['physics'])
    ...
    torch.save(model.state_dict(), "ha_model_final.pth")
    print("\n" + "="*60)
    ...
    if sim_config.get('run_after_training', True):
        run_simulation(
            ...
            save_dir=sim_config.get('output_dir', 'results')
        )
