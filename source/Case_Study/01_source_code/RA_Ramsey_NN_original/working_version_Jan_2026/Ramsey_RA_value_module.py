...
class policy_equm_funcs:
    ...
    def generate_data(self, n):
        ...
        if self.scorer is not None and self.admissibility_threshold is not None:
            ...
            g_idx_tensor = torch.tensor(g_index, device=self.device, dtype=torch.long)
            ...
            for i in range(n):  # row
                for j in range(n):  # col
                    ...
                    B_tensor = torch.tensor(B_val, device=self.device, dtype=torch.float32)
                    lam_tensor = torch.tensor(lam_val, device=self.device, dtype=torch.float32)
