...
Revised: Ensures results save to 'output/results'.
Fixed: Removed circular import inside run_simulation.
"""
...
def simulate_trajectory(model, initial_state, num_periods=50, verbose=False):
    """Simulate an equilibrium trajectory."""
    model.eval()
    device = model.device
    ...
    trajectory = {
        ...
        'welfare': [], 'fb_residuals': [], 'admissibility': []
    }
    ...
    with torch.no_grad():
        for t in range(num_periods):
            ...
            # Pass physics_out which now contains next_state
...
def run_simulation(model, boundary, config, device,
    ...
    save_dir = save_dir or sim_config.get('output_dir', 'output/results')
    ...
    # 1. Monte Carlo
    # NOTE: NO IMPORT HERE. Calling function directly.
