%% FullCourse_10Lecs -- Lecture 7, Topic 7.2b: Full Transformer Layer
%% Source: ./archive_pre_split/Lec07_LLM.tex (section 6)
%% Pass B: layer in refinements from
%%         ../../MiniCourse_8hr/Slides/Module3_LLM_Text/M3_T2b_Transformer_Architecture.tex
%%         (Vaswani 2017 + Kim et al. 10-K case study appendix)
...
Lecture 7.2b: The Full Transformer Layer Architecture}
...
%=============================================================================
\begin{frame}{Topic 7.2b Agenda}
\begin{enumerate}
  \item \textbf{Full Transformer Layer} --- residual + LayerNorm + FFN
  \medskip
  \item \textbf{Encoder, Decoder, Encoder--Decoder} --- BERT, GPT, T5; the Vaswani 2017 diagram
  \medskip
  \item \textbf{Appendix --- research case study} --- Kim, Muhn, Nikolaev, Zhang (2024): paragraph-level attention over 10-K filings
\end{enumerate}
...
%=============================================================================
\section{Encoder, Decoder, Encoder--Decoder}
...
\begin{itemize}
    ...
    \item Pretrained with \emph{next-token prediction}; autoregressive at inference (Topic 7.3a).
...
\begin{itemize}
    ...
    \item Best for: translation, summarization --- anything where input and output are clearly separated sequences.
\end{itemize}
\end{itemize}
\end{frame}
...
\section{Application: Learning Fundamentals from 10-K Text}
...
\medskip
\item \textbf{The move.} Same Q/K/V machinery from Topic 7.2a --- but the unit of analysis is a \textbf{paragraph}, not a word token. A 10-K is a sequence of $\sim 400$ paragraphs; treat each one as a ``token'' and let attention learn which paragraphs matter for the market.
...
\begin{frame}{Every Concept from This Topic, Reused}
\centering
\renewcommand{\arraystretch}{1.25}
\footnotesize
\begin{tabular}{l l}
\toprule
\textbf{Slide concept (this topic)} & \textbf{Paper usage} \\
\midrule
$Q, K, V$ projections (7.2a)                  & Same, but over \emph{paragraphs} not tokens \\
Scaled softmax $\mathrm{softmax}(QK^{\top}/\sqrt{d})V$ (7.2a) & Used verbatim in Layer 1 \\
Sinusoidal positional encoding (7.2a)         & Same Vaswani formula, scaled by embedding norm \\
Residual connection (7.2b)                    & Added to Layer 1 output (paper fn.\ 13) \\
Cosine similarity (Topic 7.1)                 & Importance score: $\mathrm{cos}(e_i,\, X^{*})$ \\
...
\item Firms strategically place bad-news / low-profitability content \textbf{later} in MD\&A (Bloomfield 2008; Cohen et al.\ 2020).
\end{itemize}
\end{columns}
\end{frame}

\begin{frame}{Take-Aways and Bridges}
\begin{enumerate}
\item \textbf{Same architecture, different ``token.''} Wherever an ordered sequence has context-dependent importance, the transformer is a tool. Macro/finance natural fits:
\begin{itemize}
    \item Paragraphs of FOMC minutes / Beige Book (monetary-policy text)
    \item Sentences of central-bank speeches (hawk--dove decomposition)
    \item Stocks in an investor's portfolio --- Gabaix, Koijen, Richmond, Yogo (2023), \emph{Asset Embeddings}
    \item Country--year observations in a macro panel
\end{itemize}

\medskip
\item \textbf{Supervised attention is a research instrument.} Once attention is trained against an economic target (returns, earnings, employment, output), the weights become inspectable measures of \emph{economic} importance --- decomposable across items, topics, time, and firm characteristics.

\medskip
\item \textbf{Bridge to Topic 7.3a.} This paper is \emph{encoder}-style supervised. Topic 7.3a covers \emph{decoder-only} generative training. The same paper actually combines both: GPT-4o (decoder LLM) discovers the topic taxonomy that the encoder then scores --- a hybrid pipeline the next topic equips you to build.
\end{enumerate}
\end{frame}
