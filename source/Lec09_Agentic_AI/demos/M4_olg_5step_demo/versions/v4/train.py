"""V4 training — capital Euler now includes the marginal adjustment-cost term."""
...
def euler_residuals(z_idx: torch.Tensor, k: torch.Tensor, b: torch.Tensor, net: PolicyNet,
    ...
    for jz in range(n_z):
        ...
        out_p = cohort_decisions(Zp, k_p, b_p, s_K_p, b_next_p, p_b_p,
                                 bonds_off=bonds_off, psi_K=psi)
        c_next = out_p["c"][..., 1:N]
        rp = out_p["r"]
    ...
    # Capital Euler with adjustment cost:
    #   u'(c) · (1 + ψ_K · (k_next - k)) = β · E[(1+r') · u'(c')]
